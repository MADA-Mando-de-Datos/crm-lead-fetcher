...
class Lead(models.Model):
    ...
    source_rating = fields.Float(
        string="Calificación del Negocio", help="Calificación promedio del negocio en la fuente de origen"
    )
    source_reviews = fields.Integer(
        string="Número de Reseñas", help="Cantidad de reseñas del negocio en la fuente de origen"
    )
    source_price_level = fields.Selection(
        ...
        help="Nivel de precio reportado por la fuente",
    )
    ...
    def action_enrich_email(self):
        ...
        is_directory_link = bool(
            self.website and any(d in self.website.lower() for d in enricher.DISCOVERY_EXCLUDE_DOMAINS)
        )
    ...
    def action_enrich_email_batch(self):
        ...
        for lead in self:
            ...
            is_dir_link = bool(
                lead.website and any(d in lead.website.lower() for d in enricher.DISCOVERY_EXCLUDE_DOMAINS)
            )
            ...
            if (
                lead.website
                and not lead.email_from
                and not any(d in lead.website.lower() for d in enricher.DISCOVERY_EXCLUDE_DOMAINS)
            ):
