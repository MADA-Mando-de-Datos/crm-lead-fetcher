...
class CrmLeadRequest(models.Model):
    ...
    lead_number = fields.Integer(
        string="Cantidad de Leads", default=10, required=True, help="Número de prospectos a generar."
    )
    ...
    estrato_min = fields.Selection(
        selection=[(str(i), ESTRATO_LABELS[i]) for i in range(1, 8)], string="Tamaño mínimo", default="1"
    )
    estrato_max = fields.Selection(
        selection=[(str(i), ESTRATO_LABELS[i]) for i in range(1, 8)], string="Tamaño máximo", default="7"
    )
    ...
    search_type = fields.Selection(
        ...
        string="Tipo de búsqueda DENUE",
        required=True,
        default="by_activity_state",
    )

    # --- Criterios YELP ---
    yelp_term = fields.Char(
        string="Término de búsqueda (Yelp)",
        help='Palabra clave (ej: "restaurante", "plomería")',
    )
    yelp_location = fields.Char(
        string="Ubicación (Yelp)",
    ...
    def _create_leads_from_response(self, records):
        ...
        existing_phones = set()
        if candidate_phones:
            existing_phones = set(Lead.search([("phone", "in", list(candidate_phones))]).mapped("phone"))

        existing_names = set()
        if candidate_names:
            existing_names = set(Lead.search([("name", "in", list(candidate_names))]).mapped("name"))
