...
from ..exceptions import LeadSourceAuthError, LeadSourceConnectionError, LeadSourceDataError
...
class DenueApi(models.AbstractModel):
    ...
    def _make_request(self, url):
        ...
        for attempt in range(self.MAX_RETRIES):
            ...
            try:
                ...
                raise LeadSourceDataError(
                    _(
                        "DENUE devolvió una respuesta no válida (HTTP %s). Revise los filtros de búsqueda.",
                        resp.status_code,
                    )
                ) from err
        token = self._token()
    ...
    def search_by_state(self, entidad, max_records, condicion="todos"):
        ...
        return self._paginate("by_state", {"condicion": self._q(condicion or "todos"), "entidad": entidad}, max_records)
    ...
    def search_by_activity_and_state(self, actividad, entidad, max_records):
        ...
        return self._paginate("by_activity_state", {"condicion": self._q(actividad), "entidad": entidad}, max_records)

    @api.model
    def search_by_name_and_state(self, nombre, entidad, max_records):
        """Búsqueda por nombre comercial + estado (endpoint Nombre)"""
        return self._paginate("by_name_state", {"nombre": self._q(nombre), "entidad": entidad}, max_records)
