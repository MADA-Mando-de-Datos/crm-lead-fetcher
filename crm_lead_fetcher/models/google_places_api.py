...
class GooglePlacesAPI(models.AbstractModel):
    ...
    def _api_key(self):
        return (self.env['ir.config_parameter'].sudo()
                .get_param('crm_lead_fetcher.google_places_api_key', '')).strip()

    @api.model
    def _location(self):
        return (self.env['ir.config_parameter'].sudo()
                .get_param('crm_lead_fetcher.google_location', 'San Luis Potosí, México')).strip()
    ...
    @api.model
    def search_text(self, query, location=None, place_type=None, radius=None,
                    min_price=None, max_price=None, page_token=None, open_now=None):
    ...
    @api.model
    def _paginate_type(self, query=None, location=None, place_type=None, radius=None,
                       min_price=None, max_price=None, max_results=None, fetch_details=True,
                       open_now=None):
    ...
    @api.model
    def paginate(self, query=None, location=None, place_types=None, radius=None,
                 min_price=None, max_price=None, max_results=None, fetch_details=True,
                 open_now=None):
        ...
        types = list(place_types) if isinstance(place_types, list) and place_types else ([place_types] if place_types else [])

        if not types:
            return self._paginate_type(
                query=query, location=location, place_type=None, radius=radius,
                min_price=min_price, max_price=max_price, max_results=max_results,
                fetch_details=fetch_details, open_now=open_now), 0
        ...
        for t in types:
            places = self._paginate_type(
                query=query, location=location, place_type=t, radius=radius,
                min_price=min_price, max_price=max_price, max_results=per_type,
                fetch_details=fetch_details, open_now=open_now)
