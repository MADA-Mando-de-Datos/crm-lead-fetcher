...
class LeadSourceGoogle(models.AbstractModel):
    ...
    def validate_filters(self, wizard):
        ...
        if not (wizard.google_query or "").strip() and not wizard.google_place_type_ids:
            raise UserError(
                _("Capture un término de búsqueda o seleccione un tipo de establecimiento para Google Places.")
            )
    ...
    def test_connection(self, config=None):
        api_key = (
            (config.google_places_api_key if config and hasattr(config, "google_places_api_key") else None)
            or self._get_api()._api_key()
            or ""
        ).strip()
        if not api_key:
        ...
        try:
            ...
            if status == "OK":
                ...
                return True, _(
                    "Conexión exitosa con Google Places. Se obtuvieron %d resultado(s) de prueba.", len(results)
                )
    ...
    def get_automatic_tag_names(self, record):
        ...
        return [f"Google: {t.replace('_', ' ')}" for t in types[:3] if t not in ("point_of_interest", "establishment")]
    ...
    def record_to_lead_vals(self, record, lead_type, team_id, user_id, tag_ids, request_id):
        ...
        record.get("business_status", "OPERATIONAL")
        types = record.get("types", [])
        ...
        ("$" * price_level) if isinstance(price_level, int) and price_level > 0 else "N/A"

        types_clean = (
            ", ".join(
                [t.replace("_", " ").capitalize() for t in types if t not in ("point_of_interest", "establishment")][:4]
            )
            or "General"
        )
        description_parts = [
    ...
    def get_scan_cap(self):
        val = int(self.env["ir.config_parameter"].sudo().get_param("crm_lead_fetcher.google_max_results", "60"))
