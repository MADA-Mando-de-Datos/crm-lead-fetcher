...
class LeadSourceYelp(models.AbstractModel):
    ...
    def test_connection(self, config=None):
        api_key = (
            (config.yelp_api_key if config and hasattr(config, "yelp_api_key") else None)
        ...
        try:
            ...
            if resp.status_code == 401:
                return False, _(
                    "API Key de Yelp inválida. Verifique su clave en https://www.yelp.com/developers/v3/manage_app"
                )
            resp.raise_for_status()
            data = resp.json()
            total = data.get("total", 0)
            return True, _(
                "Conexión exitosa. Yelp encontró %d negocios. Cuota restante: %s.",
                total,
    ...
    def record_to_lead_vals(self, record, lead_type, team_id, user_id, tag_ids, request_id):
        ...
        vals = {
            ...
            "description": Markup(
                "<p><strong>Giro:</strong> %s</p><p><strong>Reseñas:</strong> %s de 5 (%s opiniones)</p>%s"
            )
            % (
                escape(", ".join(cat_names) or "General"),
                escape(str(record.get("rating") or "0.0")),
                escape(str(record.get("review_count") or 0)),
                Markup('<p><a href="%s" target="_blank" rel="noopener noreferrer">Abrir ficha en Yelp</a></p>')
                % escape(record.get("url", ""))
                if record.get("url")
                else Markup(""),
            ),
        }
    ...
    def get_scan_cap(self):
        val = int(self.env["ir.config_parameter"].sudo().get_param("crm_lead_fetcher.yelp_max_results", "240"))
